...
function checkStrength(val) {
    ...
    const colors = ['#e53e3e', '#e53e3e', '#f6ad55', '#68d391', '#48bb78'];
    ...
    card.innerHTML = `
        <div class="opportunity-card-header">
            ...
            <div class="opportunity-meta">
                ...
                <span style="background:var(--qf-accent-light,#e8f5e9);color:var(--qf-primary);padding:2px 8px;border-radius:12px;font-size:12px;">${escapeHtml(opp.category)}</span>
            </div>
        </div>
...
// Store opportunities in memory for quick access
...
function loadOpportunities() {
    fetch('/api/opportunities', { credentials: 'include' })
        ...
        .then(({ ok, data }) => {
            ...
            grid.querySelectorAll('.opportunity-card').forEach(c => c.remove());

            if (opps.length === 0) {
                emptyState.style.display = '';
            } else {
                emptyState.style.display = 'none';
                opps.forEach(opp => grid.appendChild(renderOpportunityCard(opp)));
            }
        })
        .catch(() => {});
}
...
function viewOppDetails(id) {
    const opp = opportunitiesCache.find(o => o.id === id);
    if (!opp) return;
    const skills = (opp.skills || '').split(',').map(s => s.trim()).filter(Boolean);
...
function deleteOpp(id, btn) {
    ...
    fetch(`/api/opportunities/${id}`, { method: 'DELETE', credentials: 'include' })
        ...
        .then(({ ok, data }) => {
            ...
            if (card) card.remove();
            // Update cache
            opportunitiesCache = opportunitiesCache.filter(o => o.id !== id);
            // Show empty state if no cards left
            if (opportunitiesCache.length === 0) {
                document.getElementById('oppEmptyState').style.display = '';
            }
...
// Handle opportunity form submit â€” both CREATE and EDIT
document.getElementById('opportunityForm').addEventListener('submit', function(e) {
    ...
    if (!name || !duration || !startDate || !description || !skills || !category || !futureOpportunities) {
        showToast('Please fill all required fields');
        return;
    }

    const payload = { name, duration, start_date: startDate, description, skills, category, future_opportunities: futureOpportunities, max_applicants: maxApplicants || null };
    const isEdit = editingOppId !== null;
    ...
    fetch(url, { method, credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
        ...
        .then(({ ok, data }) => {
            if (ok) {
                ...
                showToast(data.error || 'Failed. Try again.');
            }
        })
